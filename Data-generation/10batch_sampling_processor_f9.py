...
def your_objective_function(x: np.ndarray) -> float:
    """
    十维Rastrigin函数 (F9函数)
    公式: f(x) = sum(x_i^2 - 10*cos(pi*x_i)) + 10*dim
    这是一个多峰函数，具有许多局部最小值，常用于测试优化算法
    """
...
# 选择要使用的目标函数
OBJECTIVE_FUNCTION = your_objective_function  # 使用Rastrigin函数

# --------------------- 配置参数 ---------------------
# 注意：Rastrigin函数的典型定义域是 [-5.12, 5.12]，但也可以使用其他区间
DOMAIN = {
