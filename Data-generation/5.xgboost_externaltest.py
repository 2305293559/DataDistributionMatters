...
class TabularModelTrainer:
    ...
    def train_xgboost(self):
        """
        训练 XGBoost，使用 GridSearchCV 寻找最优参数以保证收敛
        """
        param_grid = {
    ...
    def compute_metrics(self, model_name):
        """
        计算训练集、测试集和外部测试集的 R2, RMSE, MAE
        """
        model = self.models[model_name]
        
        # 定义计算各种指标的闭包
        def get_all_metrics(X_s, y_orig):
            if X_s is None: return np.nan, np.nan, np.nan
        ...
        train_r2, train_rmse, train_mae = get_all_metrics(self.X_train_scaled, self.y_train)
        val_r2, val_rmse, val_mae = get_all_metrics(self.X_val_scaled, self.y_val)
        ext_r2, ext_rmse, ext_mae = get_all_metrics(self.X_ext_test_scaled, self.y_ext_test)
        ...
        self.results[model_name].update({
            ...
            'external_test_rmse': ext_rmse,
            'external_test_mae': ext_mae,
        ...
        # print(f"Internal Test R2: {val_r2:.4f} | RMSE: {val_rmse:.4f} | MAE: {val_mae:.4f}")
        # print(f"External Test R2: {ext_r2:.4f} | RMSE: {ext_rmse:.4f} | MAE: {ext_mae:.4f}")
        # print(f"CV Mean R2: {np.mean(cv_scores):.4f}")
        print(f"{ext_r2:.4f} | {ext_rmse:.4f} | {ext_mae:.4f} | {val_r2:.4f} | {val_rmse:.4f} | {val_mae:.4f} | {train_r2:.4f} | {train_rmse:.4f} | {train_mae:.4f}")
...
if __name__ == "__main__":
    ...
    # 初始数据集文件夹
    # INPUT_FOLDER = "test"
    INPUT_FOLDER = f"BO/F{F_NUM}"
    OUTPUT_BASE = f"10dim_xgb_test_func{F_NUM}"

    csv_files = list(Path(INPUT_FOLDER).glob("*.csv"))
    # csv_files = [os.path.join(INPUT_FOLDER, "10dim_slpa_1000_fun5_kappa2.0.csv")]
    for f in csv_files:
        print(f"\n处理文件: {f.name}")
        out_path = os.path.join(OUTPUT_BASE, f.stem)
        # out_path = os.path.join(OUTPUT_BASE, f)
