...
def your_objective_function(x: np.ndarray) -> float:
    ...
    全局最小值在 x = (0,0,...,0) 处，函数值为0
    """
    x_array = np.asarray(x)
    n = len(x_array)
    
    # 第一项: 平方和
    sum1 = np.sum(x_array**2)
    
    # 第二项和第三项: 带权线性和的平方和四次方
    weighted_sum = np.sum(0.5 * np.arange(1, n+1) * x_array)
    sum2 = weighted_sum**2
    sum3 = weighted_sum**4
...
# 选择要使用的目标函数
OBJECTIVE_FUNCTION = your_objective_function  # 使用Zakharov函数

# --------------------- 配置参数 ---------------------
# Zakharov函数的典型定义域是 [-5, 10]
DOMAIN = {
    "x1": (-5, 10),
    "x2": (-5, 10), 
    "x3": (-5, 10),
