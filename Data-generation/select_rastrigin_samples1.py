"""
select_rastrigin_samples1.py

功能说明
--------
从已有的大规模 Rastrigin 随机采样数据中，按“可配置采样策略”抽取一个子集。
...
# ------------------ 采样策略配置（只改这里） ------------------
SAMPLING_CONFIG = {"high": 0.25, "low": 0.25, "random": 0.50}
...
def main():
    ...
    if n_high > 0:
        ...
        df_high_sel = df_high.sample(n=n_high_actual, replace=False, random_state=rng)
    else:
    ...
    if n_low > 0:
        ...
        df_low_sel = df_low.sample(n=n_low_actual, replace=False, random_state=rng)
    else:
    ...
    df_random_sel = df_remaining.sample(
        n=n_random, replace=False, random_state=rng
    )
