...
def your_objective_function(x: np.ndarray) -> float:
    """
    十维Rosenbrock函数 (F5函数)
    公式: f(x) = sum(100*(x_{i+1} - x_i^2)^2 + (x_i - 1)^2)
    这是一个经典的优化测试函数，具有一个狭窄的抛物线形谷
    全局最小值在x_i = 1处，函数值为0
    """
    ...
    for i in range(dim-1):
        o += 100 * (x_array[i+1] - x_array[i]**2)**2 + (x_array[i] - 1)**2
...
# 选择要使用的目标函数
OBJECTIVE_FUNCTION = your_objective_function  # 使用Rosenbrock函数

# --------------------- 配置参数 ---------------------
# Rosenbrock函数的定义域通常为 [-30, 30] 或 [-2.048, 2.048]
DOMAIN = {
    "x1": (-30, 30),
    "x2": (-30, 30), 
    "x3": (-30, 30),
