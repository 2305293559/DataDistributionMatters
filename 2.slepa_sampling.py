...
class SelfLearningPopulationAnnealing:
    ...
    #     score = 1.0 * normalize(extreme_score) + 1.96 * normalize(uncertainty_score)
    ...
    def run(self, verbose: bool = True):
        ...
        n_warmup = n_annealing_steps // 10  # 前一半轮数用于纯探索（空间填充）
        ...
        # 初始化群体 (M 个粒子)
        population = rng.uniform(bounds[:, 0], bounds[:, 1], (n_init, dim))
        ...
        for i in range(len(betas) - 1):
            ...
            if np.sum(weights) > 0:
                ...
                weights = np.ones(n_init) / n_init
            
            # 依据权重重采样群体
            indices = rng.choice(n_init, size=n_init, p=weights)
            ...
            for _ in range(n_mcmc_steps):
                # 提议新位置：高斯扰动
                perturbation = rng.normal(0, 1, (n_init, dim)) * step_scale
            ...
            candidates_idx = rng.permutation(n_init)
            new_X = []
            new_y = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--population_size", type=int, default=300, help="群体大小")
    ...
    F_NUM = 5
    SAMPLE_NUM_LIST = [1000]  # 采样点数列表
    # SAMPLE_NUM_LIST = [1000]
    # 默认定义域：10 维
    ...
    for n_samples in SAMPLE_NUM_LIST:        # 执行 SLPA 采样
        ...
        n_offspring=min(args.n_offspring, population_size)
        ...
        # 生成输出文件名
        out_file = args.out or f"./test/10dim_slpa_{n_samples}_fun{F_NUM}_kappa2.0.csv"
